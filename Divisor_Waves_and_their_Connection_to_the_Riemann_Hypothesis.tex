...
expanded as,
\begin{align*}
	Q_1(z) = (-1)^{2^{J(2)}}(-1)^{2^{J(3)}}(-1)^{2^{J(4)}}\ldots(-1)^{2^{J(z)}}
\end{align*}

where J(z) is the Binary Output Indicator Function (BOPIF), leveraging its oscillation between 1 and 0 at prime and composite numbers, and ultimately executing the rule, $(2)^{0}=(1) & (2)^{1}=(2)$, this can be see in the following
